...
class Feture_3d(nn.Module): #3d特征
    def __init__(self, args):
        super(Feture_3d, self).__init__()
    ...
    def forward(self, smiles):
        ...
        for one in smiles:
            v = smile2volume(one)
            v_outs.append(v)
        v_outs = torch.stack(v_outs, dim=0)
        if self.cuda:
            v_outs = v_outs.cuda()
        return v_outs
...
class MoleculeFormerModel(nn.Module): #组合的方法 ，里面加vit transformer
    def __init__(self, is_classif, gat_scale, cuda, dropout_fpn):
        super(MoleculeFormerModel, self).__init__()
        ...
        self.cls_token = nn.Parameter(torch.randn(1, 1, 100)) # 初始cls
    ...
    def create_3d(self, args):
        self.encoder5 = Feture_3d(args)
    ...
    def create_ffn(self, args):  #
        ...
        self.ffn = nn.Sequential(
            ...
            nn.Linear(in_features=linear_dim*2+6, out_features=linear_dim, bias=True),
            nn.ReLU(),
            nn.Dropout(self.dropout_fpn),
    ...
    def forward(self, input): # 定义transformer
        ...
        v_out = self.encoder5(input)
        v_out = self.act_func(v_out) #
        ...
        padded_batch_data = torch.cat([fpn_out,transformer_out,v_out], axis=1)
...
def MoleculeFormer(args): #最后结合
    if args.dataset_type == 'classification':
        is_classif = 1
    else:
        is_classif = 0
    model = MoleculeFormerModel(is_classif, args.gat_scale, args.cuda, args.dropout)

    model.create_transformer(args)
    model.create_fpn(args)
    model.create_gcn(args)
    model.create_3d(args) #加

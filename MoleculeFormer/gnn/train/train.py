...
from gnn.tool.tool import mkdir, get_task_name, load_data, split_data, get_label_scaler, get_loss, get_metric, save_model, NoamLR, load_model,choose_model
from gnn.model import FPGNN,CAF3DGNN
...
def predict(model,data,batch_size,scaler):
    ...
    # for i in range(0,data_total,batch_size):
        try:
            ...
            print(f"An error 无法解析，返回空{str(e)}",smile)
            pred_now=[[-1]]
            pred.extend(pred_now)
...
def fold_train(args, log):
    ...
    if args.val_path and args.test_path:
        ...
        train_data, val_data, test_data = split_data(data,args.split_type,args.split_ratio,args.seed,log) # 分割
    ...
    for epoch in range(args.epochs):
        ...
        # train_pred = predict(model,train_data,args.batch_size,label_scaler) #train_pred
        # train_label = train_data.label()
        # train_score = compute_score(train_pred,train_label,metric_f,args,log)
        val_pred = predict(model,val_data,args.batch_size,label_scaler) # val predict
        ...
        if data_type == 'classification' and ave_val_score > best_score:
            ...
            best_epoch = epoch
            save_model(os.path.join(args.save_path, 'model.pt'),model,label_scaler,args) #如果分类验证集auc》，保存pt
    ...
    model = load_model(os.path.join(args.save_path, 'model.pt'),args.cuda,log) # 用最好的auc，使用测试集测试正确率

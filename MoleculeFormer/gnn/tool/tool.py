...
from gnn.data import MoleDataSet, MoleData, scaffold_split,ratio_split

from gnn.model import FPGNN,CAF3DGNN,TransformerGraph,onlyFP,GATGNN,GCNGNN,FPGCN,TransformerVit,TransformerVitno3d,TransformerVitno3d300,GCNTransformer,v2,v2no3d
...
def choose_model(args):
    print("选择模型：",args.model_type)
    if args.model_type == "FPGNN":
        return FPGNN(args)

    elif args.model_type == "v1": # 没有transofrmer
        return CAF3DGNN(args)

    elif args.model_type == "transformer": # transformer pooling + GAT + GCN
        return TransformerGraph(args)

    elif args.model_type == "FP": #只有分子指纹的消融实验
        return onlyFP(args)
    ...
    elif args.model_type == "FPGCN": #GCN拼FP
        return FPGCN(args)

    elif args.model_type == "Vit": # GCN + Transformer 随机cls
        return TransformerVit(args)

    elif args.model_type == "no3d": # GCN + Transformer 随机cls，减去3d
        return TransformerVitno3d(args)

    elif args.model_type == "no3d300": # GCN + Transformer 随机cls，减去3d，hidden_size = 300 维度
        return TransformerVitno3d300(args)

    elif args.model_type == "GCNtransformer": # 只用GCN+Transformer 消融减去FP
        return GCNTransformer(args)

    elif args.model_type == "MoleculeFormer": # 只用GCN+Transformer， cls 可学习消融减去FP
        return MoleculeFormer(args)

    elif args.model_type == "MoleculeFormerno3d": # 只用GCN+Transformer， cls 可学习消融减去FP
        return MoleculeFormerno3d(args)

    if args.model_type == "":
        print("choose model use:--model_type MoleculeFormer")
        assert "请选择模型 model_type == MoleculeFormer"
